...
> 라우팅·검색·합성은 watsonx.ai granite-3-8b 직접 호출. 같은 도구를 **Langflow에서 시각적으로
> 오케스트레이션**하는 병렬 데모도 있다(아래 "Langflow 병렬 데모"). 답변 언어는 질문 언어를 따른다(한글→한글).
...
oc -n genai-apps create configmap rag-code   --from-file=ingest.py --from-file=reindex.py \
  --from-file=rag_common.py --from-file=agent.py --from-file=app.py \
  --from-file=text2sql.py --from-file=seed_aml.py --dry-run=client -o yaml | oc apply -f -
...
## Langflow 병렬 데모 (오케스트레이션 시각화)

파이썬 엔진이 챗 UI를 구동하고, **Langflow(1.8.0)에서 동일 도구를 에이전트가 tool-calling으로 고르는
플로우**를 시각적으로 시연한다. rag-ui가 도구 엔드포인트를 노출(토큰 게이트):
...
- 헤더 `X-Tool-Token: <APP_PASSWORD>`(미설정 시 `TOOL_TOKEN`), 내부 URL
  `http://rag-ui.genai-apps.svc.cluster.local:8080`.

**완성 플로우**: `ChatInput → Agent(IBM watsonx.ai, granite-4-h-small) → ChatOutput`, Agent가 3개
커스텀 툴(`search_documents`/`lookup_relationships`/`query_business_data`)을 tool-calling으로 선택.
라이브 검증됨("위험등급 high 고객 수?" → query_business_data 호출 → "7명"). 시크릿 스크럽본:
