...
> 챗 UI(`/api/chat`)는 오케스트레이션을 **Langflow의 watsonx granite 에이전트**(tool-calling 루프)에 위임하며,
> 장애 시 in-process **파이썬 에이전트(단일 패스 라우터)로 폴백**합니다.
...
| 오케스트레이션 | **Langflow** 에이전트(watsonx granite, tool-calling) — 챗 UI 메인 엔진(파이썬 폴백) |
...
| 도구 | 데모(현재) | 프로덕션 경로 |
|---|---|---|
| 오케스트레이션 | Langflow 에이전트(tool-calling) 메인 + 파이썬 폴백 | LangGraph plan→reflect→replan 루프, 체크포인트·HITL |
...
```
질문 → 챗 UI(/api/chat) → Langflow 에이전트(watsonx granite, tool-calling)  ※장애 시 파이썬 폴백
